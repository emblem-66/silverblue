...
# && sed -i 's/#AutomaticUpdatePolicy=none/AutomaticUpdatePolicy=stage/' /etc/rpm-ostreed.conf \
 && echo -e "[Unit]\nDescription=Bootc Update\nConditionPathExists=/run/ostree-booted/n\n[Service]\nType=oneshot\nExecStart=/usr/bin/bootc update" | tee etc/systemd/system/bootc-update.service \
 && echo -e "[Unit]\nDescription=Update Bootc\nConditionPathExists=/run/ostree-booted\nAfter=multi-user.timer\n\n[Timer]\nOnCalendar=*:0/4\nPersistent=true\n[Install]\nWantedBy=timers.target\n" | tee etc/systemd/system/bootc-update.timer \
